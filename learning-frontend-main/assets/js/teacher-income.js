...
function renderPieChart(walletLogs, bookings) {
    ...
    const incomeItems = walletLogs.filter(log => log.transactionType === 3);
    ...
    // 依課程分組加總
    const courseIncome = {};
    incomeItems.forEach(log => {
        const courseName = bookingMap[log.relatedId] || '其他';
        courseIncome[courseName] = (courseIncome[courseName] || 0) + Number(log.amount);
    });

    const labels = Object.keys(courseIncome);
    const data   = Object.values(courseIncome);
...
// ── 渲染收入明細 ──
function renderIncomeList(walletLogs, bookings) {
    const container = document.getElementById('income-list');
    const bookingMap = {};
    bookings.forEach(b => { bookingMap[b.id] = b; });

    const incomes = walletLogs
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        const monthIncome = statsRes.data.monthIncome || 0;
        const walletLogs  = logsRes.data;
        const bookings    = bookingsRes.data;

        // 計算累計總收入
        const totalIncome = walletLogs
            .filter(log => log.transactionType === 3)
            .reduce((sum, log) => sum + Number(log.amount), 0);

        // 更新統計卡片
        document.getElementById('stat-wallet').textContent       = formatMoney(wallet);
        document.getElementById('stat-month-income').textContent = formatMoney(monthIncome);
        ...
        renderIncomeList(walletLogs, bookings);
